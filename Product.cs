...
namespace Exam_Semester2
{
    class Product
    {
        ...
        internal delegate void IsSpoilageEventHandler(object obj, EventArgs e,Product pd);
        internal event IsSpoilageEventHandler IsSpoilage;
        internal event IsSpoilageEventHandler IsNotSpoilage;
        ...
        internal void DaysLeft(int _day,int _month,int _year)
        {
            int daysleft = 0;
            daysleft = (this.date.year - _year) * 12 * 30 + (this.date.month - _month) * 30 + (this.date.day - _day);
            if (daysleft <= 0)
